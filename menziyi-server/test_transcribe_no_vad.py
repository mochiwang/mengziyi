...
"""
测试脚本：不使用VAD过滤的转写测试
使用方法：python test_transcribe_no_vad.py <wav文件路径>
"""
...
def transcribe_wav_no_vad(wav_path):
    """转写单个 WAV 文件，不使用VAD过滤"""
    if not os.path.exists(wav_path):
        log.error(f"文件不存在: {wav_path}")
        return None
    
    try:
        log.info(f"开始转写（无VAD过滤）: {wav_path}")
        ...
        txt_path = f"{base_name}_no_vad.txt"
        json_path = f"{base_name}_no_vad.segments.json"
        ...
        if lines:
            ...
            for i, line in enumerate(lines[:5]):
                log.info(f"  {i+1}. {line}")
            if len(lines) > 5:
                log.info(f"  ... 还有 {len(lines)-5} 行")
        else:
            log.warning("没有检测到任何文本内容！")
...
def main():
    if len(sys.argv) != 2:
        print("使用方法: python test_transcribe_no_vad.py <wav文件路径>")
        print("示例: python test_transcribe_no_vad.py data/lessons/2025-08-19/20250819-222425-0e5ca398.wav")
        sys.exit(1)
    
    wav_path = sys.argv[1]
    result = transcribe_wav_no_vad(wav_path)
